...
# Download SM 1.10 for compilation (compiler + includes)
...
SM110_INCLUDE="${ZR_TEMP}/addons/sourcemod/scripting/include"

# Copy ZR include files into SM 1.10's include dir for compilation
cp "${ZR_SRC}/include/"*.inc "${SM110_INCLUDE}/"
cp -r "${ZR_SRC}/include/zr" "${SM110_INCLUDE}/"

# Compile plugins from source using SM 1.10 compiler + includes
for sp in "${ZR_SRC}/zombiereloaded.sp" "${ZR_SRC}/zombiereloaded_sounds.sp"; do
    name="$(basename "${sp}" .sp)"
    echo "    Compiling ${name}.sp..."
    "${SPCOMP}" -i"${SM110_INCLUDE}" -i"${ZR_SRC}" "${sp}" -o"${SM_DIR}/plugins/${name}.smx"
done
